...
const firebaseConfig = {
  ...
  var Email = { send: function (a) { return new Promise(function (n, e) 
    ...
    let dragName =document.getElementById("file-name");
    /*
    let fileName;
...
input.onchange = (e) => {
    ...
    document.getElementById('submit-button').addEventListener('click', ()=>{
        fileName = dragName.innerHTML;
                    codecontainer.push(results.data[0].COURSE_CODE);
                    codecontainer = [...new Set(codecontainer)];

                    if(results && Array.isArray(results.data)){
                        let firstCourseCode = results.data[0] ? results.data[0].COURSE_CODE : null;
                        let allCourseCodes = results.data.map(item => item.COURSE_CODE);
                        
                        // Start the loop from index 1 to ignore the first data
                        for(let x = 1; x < allCourseCodes.length; x++){
                            if(allCourseCodes[x] !== firstCourseCode){
                                document.getElementById('pop-up-top').style="display: none";
                                document.getElementById('pop-up-message').innerHTML="Its seems that their is among us in COURSE_CODE, Please Check it thank you";
                                document.getElementById('pop-up-message').style.textAlign = "center";
                                myPopup.classList.add("show");
                                cleanUp();
                            }else{
                                const ref = doc(db, "PENDING-STUD-DATA",fileName);
                            setDoc(
                            ref, {
                                Alldata : results,
                        ...
                        var tref = doc(db, "ADMIN_LIST", adminID);
                                ...
                                document.getElementById('pop-up-top').style="display: none";
                                document.getElementById('pop-up-message').innerHTML="Data uploaded succesfully, Please be informed that the data will not be automatically distribute, it will go to in academic approval section, Thank you!";
                                document.getElementById('pop-up-message').style.textAlign = "center";
                                myPopup.classList.add("show");
                                cleanUp();
                            }
                        ...
                        console.log('results or results.data is undefined or not an array');
                    }
    })


        async function transferData() {
            const ref = doc(db, "ADMIN_LIST", adminID);
                const admdocsnap = await getDoc(ref);
                if (admdocsnap.exists()) {
                    tname.value = admdocsnap.data().TeacherName;
                    email = admdocsnap.data().Email_Address;
                    allsubs.push(admdocsnap.data().subjects);
                    ...
                    pass = admdocsnap.data().password;
                    allsubs = admdocsnap.data().subjects;
